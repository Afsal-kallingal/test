...
#         request_data = request.data.copy()
#         request_data['email'] = my_session['email']
#         request_data['username'] = my_session['email']
#         request_data['password'] = my_session['password']
#         serializer = UserSerializer(data=request_data)
#         if serializer.is_valid():
#             user = serializer.save()
...
def login_email_pass(request):
    ...
    password = request.data.get('password')
    if(User.objects.filter(email=email, email_verified=True,is_admin=True).exists):
        ...
        if user.check_password(password):
            ...
            response_data = {'user_id': user.pk, 'token': token,'role':'admin'}
            return Response(response_data, status=status.HTTP_200_OK)
...
def login_phone_otp(request):
    ...
    try:
        user = User.objects.get(country_code=country_code,phone=phone, phone_verified=True)
...
@permission_classes((AllowAny, ))
@parser_classes([JSONParser, FormParser, MultiPartParser, FileUploadParser])
def resend_login_phone_otp(request):
    session_key = request.data.get('session_key')
    try:
        my_session = SessionStore(session_key=session_key)
    except Session.DoesNotExist:
        return Response({'detail': 'Invalid session key'}, status=status.HTTP_400_BAD_REQUEST)
    try:
        phone = my_session['phone']
        country_code = my_session['country_code']
        otp = my_session['otp']
        # my_session.create()
        send_phone_otp(country_code,phone,1212)
...
def logout_view(request):
    context = {}
    try:
        # delete_user_token(request.user.auth_token.key)
...
class LoginHistoryViewSet(BaseModelViewSet):
    permission_classes = [IsAdmin]
